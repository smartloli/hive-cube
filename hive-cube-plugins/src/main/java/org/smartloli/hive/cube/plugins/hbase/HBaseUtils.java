...
/**
 * The HBase tool handles classes for handling HBase interface access.
 * 
 * @author smartloli.
 *
...
public class HBaseUtils {
	...
	static Filter parseFilterString(String filterString) {
		...
		try {
			...
			throw new HBaseRuntimeException("Error parsing filter string: " + filterString, e);
		}
	}
	
	public static Object convertFrom(byte[] bytes) {
		Object obj = null;
		if (bytes.length == 4)
			obj = Bytes.toInt(bytes);
		else if (bytes.length == 8)
			obj = Bytes.toLong(bytes);
		else
			obj = Bytes.toString(bytes);
		return obj;
	}
	...
	public static byte[] serializeFilter(Filter filter) {
		...
		try {
			...
			throw new HBaseRuntimeException("Error serializing filter: " + filter, e);
		}
	}
	...
	public static Filter deserializeFilter(byte[] filterBytes) {
		...
		try {
			...
			throw new HBaseRuntimeException("Error deserializing filter: " + filterBytes, e);
		}
	}
