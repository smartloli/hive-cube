...
public class RoleController {
	...
	public void getUserRoleAjax(@PathVariable("userId") int userId, HttpServletResponse response, HttpServletRequest request) {
		try {
	...
	public void transformUserRoleAjax(@PathVariable("action") String action, @PathVariable("userId") int userId, @PathVariable("roleId") int roleId, HttpServletResponse response,
		...
		try {
			...
			if ("add".equals(action)) {
				...
				if (code > 0) {
					object.put("info", "Add permission success.");
				} else {
					object.put("info", "Add permission failed.");
				}
			} else if ("delete".equals(action)) {
				code = roleService.deleteUserRole(userRole);
				if (code > 0) {
					object.put("info", "Delete permission success.");
				} else {
					object.put("info", "Delete permission failed.");
				}
			}
	...
	/** Transform role resource by role id. */
	@RequestMapping(value = "/role/{action}/{roleId}/{resourceId}/", method = RequestMethod.GET)
	public void transformRoleResource(@PathVariable("action") String action, @PathVariable("roleId") int roleId, @PathVariable("resourceId") int resourceId,
